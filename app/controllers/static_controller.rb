class StaticController < ApplicationController
	def home
		...
		@top_elevation_per_mile = User.where(["ytd_mileage >= ?", 50]).sort{|a,b| (b.ytd_elevation / b.ytd_mileage) <=> (a.ytd_elevation / a.ytd_mileage) }[0..9]
		@newest_members = User.limit(5).order("created_at DESC")
		@top_trips = Trip.limit(5).where("")		
